...
const App = () => {
    ...
    const calculateTotalPower = () => {
        const calculateVesselPower = (components) => {
            if (components.length === 0) return 0;

            let maxPower = 0;
            let sumPower = 0;
            let weightedCombinabilitySum = 0;

            components.forEach(c => {
                if (c.power > maxPower) maxPower = c.power;
                sumPower += c.power;

                const combinability = c.combinability !== undefined ? c.combinability : 0.75;
                weightedCombinabilitySum += c.power * combinability;
            });

            if (sumPower === 0) return 0;

            const avgCombinability = weightedCombinabilitySum / sumPower;
            const vesselPower = maxPower * Math.pow((sumPower / maxPower), avgCombinability);

            return vesselPower * rangeModifier;
        }

        return {
            antennaTotal: calculateVesselPower(antennas),
            relayTotal: calculateVesselPower(relays)
        }
    }
                ...
                updateAntennas(
                    [
                        ...antennas,
                        {
                            ...
                            combinability: antennaData[id].combinability
                        }
                    ]
                )
                break;
                ...
                updateRelays(
                    [
                        ...relays,
                        {
                            ...
                            combinability: relayData[id].combinability
                        }
                    ]
                )
                break;
